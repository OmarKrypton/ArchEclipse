...
static void create_config_structure() {
  ...
  snprintf(backup_conf, sizeof(backup_conf),
           "%s/hyprpaper/config/defaults.conf", hypr_dir);

  if (!ensure_dir(base_dir)) {
    notify_error("create_config_structure",
                 "Failed to ensure base config directory");
    return;
  }
  ...
  while (line) {
    ...
    snprintf(monitor_conf, sizeof(monitor_conf), "%s/defaults.conf",
             monitor_dir);

    if (!ensure_dir(monitor_dir)) {
      char error_msg[512];
      snprintf(error_msg, sizeof(error_msg), "Failed to create dir for %s",
               line);
    ...
    if (needs_init) {
      if (!copy_file(backup_conf, monitor_conf)) {
        char error_msg[512];
        snprintf(error_msg, sizeof(error_msg), "Failed to init %s",
                 monitor_conf);
        notify_error("create_config_structure", error_msg);
      }
    }
...
void notify_error(const char *where, const char *message) {
  ...
  snprintf(cmd, sizeof(cmd),
           "notify-send -u critical 'Wallpaper Daemon Error' '%s: %s'", where,
           message);
  system(cmd);
...
char *exec_command(const char *cmd) {
  ...
  if (!fp) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "popen failed for '%s': %s", cmd,
             strerror(errno));
 ...
 */
bool get_wallpaper_for_workspace(const char *monitor, int workspace_id,
                                 char *wallpaper, size_t size) {
  char config_path[MAX_PATH_LEN];
  snprintf(config_path, sizeof(config_path),
           "%s/hyprpaper/config/%s/defaults.conf", hypr_dir, monitor);

  FILE *fp = fopen(config_path, "r");
  if (!fp) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Cannot open config at %s: %s",
...
int get_active_workspace(const char *monitor_name) {
  ...
  if (!output) {
    ...
    snprintf(error_msg, sizeof(error_msg),
             "hyprctl command failed for monitor '%s'", monitor_name);
  ...
  if (!monitor_pos) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Monitor '%s' not found in JSON",
             monitor_name);
    notify_error("get_active_workspace", error_msg);
    return -1;
  }
  ...
  if (!workspace_pos) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "activeWorkspace not found for '%s'",
             monitor_name);
    notify_error("get_active_workspace", error_msg);
    return -1;
  }
  ...
  if (!id_pos) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Workspace ID not found for '%s'",
             monitor_name);
    notify_error("get_active_workspace", error_msg);
    return -1;
  }
  ...
  char error_msg[512];
  snprintf(error_msg, sizeof(error_msg),
           "Failed to parse workspace ID for '%s'", monitor_name);
  notify_error("get_active_workspace", error_msg);
  return -1;
}
...
int get_monitors(char monitors_list[][MAX_MONITOR_NAME]) {
  ...
  while (count < MAX_MONITORS) {
    ...
    char *after_id = pos + 4;
    while (*after_id && (*after_id == ' ' || *after_id == ':'))
      after_id++;
...
void kill_wallpaper_script() {
  ...
  snprintf(cmd, sizeof(cmd), "pgrep -f '%s/hyprpaper/w.sh' >/dev/null 2>&1",
           hypr_dir);
  if (system(cmd) == 0) {
 ...
 * Iterates through all monitors, checks workspace changes, and updates
 * wallpapers Only changes wallpaper when workspace or wallpaper path differs
 * from previous state
 */
void change_wallpaper() {
  ...
  for (int i = 0; i < num_monitors; i++) {
    ...
    if (!state) {
      char error_msg[512];
      snprintf(error_msg, sizeof(error_msg),
               "Max monitors (%d) reached for '%s'", MAX_MONITORS, monitor);
    ...
    if (workspace_id == -1) {
      char error_msg[512];
      snprintf(error_msg, sizeof(error_msg),
               "Failed to get workspace ID for '%s'", monitor);
      notify_error("change_wallpaper", error_msg);
      continue;
    }
    ...
    if (!get_wallpaper_for_workspace(monitor, workspace_id, wallpaper,
                                     sizeof(wallpaper))) {
      char error_msg[512];
      snprintf(error_msg, sizeof(error_msg),
               "No wallpaper config for '%s' workspace %d", monitor,
               workspace_id);
      notify_error("change_wallpaper", error_msg);
      continue;
    }
    ...
    if (state->initialized &&
        strcmp(wallpaper, state->current_wallpaper) == 0) {
    ...
    snprintf(current_conf, sizeof(current_conf),
             "%s/hyprpaper/config/current.conf", hypr_dir);
    ...
    if (fp) {
      ...
      char error_msg[512];
      snprintf(error_msg, sizeof(error_msg), "Failed to write %s: %s",
               current_conf, strerror(errno));
      notify_error("change_wallpaper", error_msg);
    }
    ...
    snprintf(cmd, sizeof(cmd), "%s/hyprpaper/w.sh '%s' '%s' &", hypr_dir,
             monitor, expanded_wallpaper);
    system(cmd);
...
int connect_to_hyprland_socket() {
  ...
  if (!runtime_dir || !hypr_instance) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg),
             "%s not set - ensure Hyprland is running",
  ...
  if (sock < 0) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Socket creation failed: %s",
             strerror(errno));
    notify_error("connect_to_hyprland_socket", error_msg);
    return -1;
  }
  ...
  if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Connection to %s failed: %s",
...
int main() {
  ...
  if (!sock_file) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "fdopen failed: %s",
             strerror(errno));
