...
static void get_current_date(char *date_str) {
  ...
  snprintf(date_str, DATE_LEN, DATE_FORMAT, tm.tm_year + 1900, tm.tm_mon + 1,
           tm.tm_mday);
}
...
static inline int fast_parse_dev(const char *line, const char *iface,
                                 BandwidthData *out) {
...
static void read_today_bandwidth(const char *path, BandwidthData *today) {
...
static void update_today_bandwidth(const char *path,
  ...
  if (file) {
    ...
    while (fgets(line, sizeof(line), file)) {
      if (sscanf(line, "%10s", date) == 1 && strcmp(date, current_date) != 0) {
        fputs(line, temp);
      }
    }
    fclose(file);
...
int main() {
  ...
  while (1) {
    ...
    BandwidthData speed = {(unsigned long long)(d_rx / dt),
